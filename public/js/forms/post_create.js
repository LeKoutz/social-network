...
export function attachPostCreateListener(options = { editing: false }) {
        ...
        if (form) {
            ...
            form.addEventListener('submit', async (e) => {
                    ...
                    const response = await fetch(url, { // TODO: This pattern is repeated maybe it could be a function requestPOST(url) returns data? But it would require an option to use URLSearchParams if the fetch uses ParseForm or not if it uses ParseMultipartForm
                            method: 'POST',
